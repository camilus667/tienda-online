...
const ProductDetail: React.FC<ProductDetailProps> = ({ products, onAddToCart, loading }) => {
  ...
  return (
    <>
      ...
      <button 
        onClick={handleBack}
        className="fixed top-[68px] left-1 z-40 flex items-center gap-1 px-3 py-1.5 bg-white/90 backdrop-blur-md border border-gray-200 shadow-md rounded-full text-gray-700 text-sm font-medium hover:bg-gray-50 hover:pl-2 hover:pr-4 transition-all active:scale-95 group"
      >
        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
        <span>Volver</span>
      </button>

      <div className="min-h-screen pt-24 pb-10 px-2 sm:px-4 flex items-center justify-center bg-gray-50">
        <div className="bg-white rounded-2xl w-full max-w-4xl overflow-hidden shadow-xl flex flex-col border border-gray-100">
          <div className="px-4 py-3 sm:p-4 flex justify-between items-center border-b border-gray-100 flex-shrink-0">
              <h2 className="text-xl sm:text-3xl font-bold text-gray-900 leading-tight">{product.nombre}</h2>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 md:gap-8">
                  ...
                  <div className='flex flex-col justify-start md:col-span-1'> 
                      <div>
                          ...
                          <p className="text-gray-600 text-sm mb-4 leading-relaxed">{product.descripcion || "Sin descripción."}</p>
                          <div className='py-4 my-2 border-t border-b border-gray-100'> 
                            {hasVariants && (
                                <div className="mb-6"> 
                            ...
                            <div className="mb-3"> 
                                ...
                                <div className="flex flex-row items-center gap-4">
                                    <div className="flex items-center gap-2 bg-gray-50 p-1 rounded-full border border-gray-200"> 
                                        <button onClick={() => setQuantity(q => Math.max(1, q - 1))} disabled={quantity <= 1} className="p-2 rounded-full bg-white hover:bg-gray-100 text-gray-600 disabled:opacity-50 shadow-sm active:scale-95"><Minus className="w-4 h-4" /></button>
                                        <span className="text-base font-bold w-8 text-center">{quantity}</span>
                                        <button onClick={() => setQuantity(q => q < product.stock ? q + 1 : q)} disabled={quantity >= product.stock} className="p-2 rounded-full bg-white hover:bg-gray-100 text-gray-600 disabled:opacity-50 shadow-sm active:scale-95"><Plus className="w-4 h-4" /></button>
                                    </div>
                      ...
                      <div className="mt-auto pt-4">
                        <button 
                            ...
                            className={`
                                ...
                                ${hasStock && (!hasVariants || selectedSize) ? 'bg-primary text-white hover:bg-primary-dark hover:-translate-y-1' : 'bg-gray-200 text-gray-500 cursor-not-allowed'}
                            `}
                        >
