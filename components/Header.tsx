...
const Header: React.FC<HeaderProps> = ({ cartCount, onCartClick, onReload }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md border-b border-gray-100 h-16 transition-all duration-300">
      <div className="max-w-6xl mx-auto px-4 h-full flex justify-between items-center relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <h1 
                className="text-xl sm:text-2xl font-extrabold tracking-tight title-font cursor-pointer pointer-events-auto text-center px-2 select-none" 
                onClick={handleLogoClick}
            >
                {STORE_NAME}
            </h1>
        ...
        <div className="flex items-center z-20 relative">
          ...
          >
              ...
              {cartCount > 0 && (
                  <span className="absolute top-0 right-0 bg-red-500 text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full shadow-lg border-2 border-white animate-in zoom-in duration-300">
                    {cartCount}
                  </span>
              )}
          </button>
