...
const CartDrawer: React.FC<CartDrawerProps> = ({ 
  ...
  return (
    <>
        ...
        />
        <div 
            className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h2 className="text-lg font-bold flex items-center gap-2"><ShoppingCart className="w-5 h-5 text-primary" /> Tu Pedido</h2>
          <button onClick={onClose} className="p-2 hover:bg-gray-200 rounded-full transition-colors active:scale-95"><X className="w-6 h-6 text-gray-500" /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {cart.length === 0 ? (
            ...
            cart.map(item => (
                <div key={item.key} className="flex gap-4 bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                    <img 
                        src={item.imagen} 
                        alt={item.nombre} 
                        className="w-16 h-16 rounded-lg object-cover object-top bg-gray-100 flex-shrink-0" 
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between gap-2 mt-2">
                            <select 
                                ...
                                className="text-xs px-2 py-1 rounded-md border border-gray-300 bg-white shadow-sm focus:border-primary focus:ring-primary"
                            >
                                {item.tallas && item.tallas.map(size => <option key={size} value={size}>Talla {size}</option>)}
                            </select>
                            <div className="flex items-center gap-1">
                                <button onClick={() => updateQuantity(item.key, -1)} className="p-1 rounded-md bg-gray-100 hover:bg-gray-200 text-gray-600 active:scale-95"><Minus className="w-3 h-3" /></button>
                                <span className="text-sm font-medium w-4 text-center">{item.quantity}</span>
                                <button onClick={() => updateQuantity(item.key, 1)} className="p-1 rounded-md bg-gray-100 hover:bg-gray-200 text-gray-600 active:scale-95"><Plus className="w-3 h-3" /></button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-gray-100 bg-white pb-safe">
            <div className="flex justify-between items-center mb-4"><span className="text-gray-500">Total estimado</span><span className="text-2xl font-bold text-gray-900">{CURRENCY} {cartTotal}</span></div>
